...
from layers.contrast import Attention
...
class MVCL(nn.Module):
    def __init__(self, hid_dim, args, keys, mps_len_dict: dict, attn_drop, feat_dim: dict, g):
        ...
        self.attention = Attention(hid_dim, num_heads=4, attn_vec_dim=64)
    ...
    def forward(self, drug_drug, drug_chemical, drug_disease, drug_sideeffect, drug_substituents, protein_protein,
        ...
        node_fused = {}
        for key in node_ne.keys():  
            node_fused[key] = self.attention([node_ne[key], node_mp[key], node_df[key]])

        
        disease_vector = node_fused[disease]
        drug_vector = node_fused[drug]
        protein_vector = node_fused[protein]
        sideeffect_vector = node_fused[sideeffect]
        go_vector = node_fused[go]
        substituents_vector = node_fused[substituents]
