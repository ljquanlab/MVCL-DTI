...
def parse_args():
    parser = argparse.ArgumentParser(description='MVCL-DTI')
    ...
    parser.add_argument('--task', type=str, default='benchmark',
                        help='实验分为正常数据benchmark,其他是disease,drug,homo_protein_drug,sideeffect,unique等')
    parser.add_argument('--edge_mask', type=str, default='',
                        help='控制实验中是否要mask某些边 drug protein drug,protein disease sideeffect disease,sideeffect'
                             'go substituents go,substituents drugsim proteinsim drugsim,proteinsim')
