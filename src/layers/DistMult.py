...
class DistMult(nn.Module):
    def __init__(self, dim_embedding):
        ...
        self.re_D_Su = nn.Parameter(th.diag(th.nn.init.normal_(tmp, mean=0, std=0.1)))
        ...
        self.re_P_G = nn.Parameter(th.diag(th.nn.init.normal_(tmp, mean=0, std=0.1)))

    def forward(self, drug_vector, disease_vector, sideeffect_vector, protein_vector, go_vector, substituents_vector, 
        ...
        drug_substituents_reconstruct = th.mm(th.mm(drug_vector, self.re_D_Su), substituents_vector.t())
        drug_substituents_reconstruct_loss = th.sum(
            (drug_substituents_reconstruct - drug_substituents.float()) ** 2)
        ...
        protein_go_reconstruct = th.mm(th.mm(protein_vector, self.re_P_G), go_vector.t())
        protein_go_reconstruct_loss = th.sum(
            (protein_go_reconstruct - protein_go.float()) ** 2)
        ...
        DTI_potential = drug_protein_reconstruct
        ...
        if edge_mask == 'drug':
            ...
            protein_go_reconstruct_loss = 0
        elif edge_mask == 'substituents':
            drug_substituents_reconstruct_loss = 0
        ...
        other_loss = drug_drug_reconstruct_loss + drug_chemical_reconstruct_loss + drug_disease_reconstruct_loss + \
                     drug_sideeffect_reconstruct_loss + drug_substituents_reconstruct_loss + protein_protein_reconstruct_loss + \
                     protein_sequence_reconstruct_loss + protein_disease_reconstruct_loss + protein_go_reconstruct_loss
