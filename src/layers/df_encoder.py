...
class DfEncoder(nn.Module):
    def __init__(self, out_dim, g):
        ...
        # Define the transformation layers (same as before)
    ...
    def forward(self, drug_drug, drug_chemical, drug_disease, drug_sideeffect, drug_substituents, protein_protein,
        ...
        # Add residual connection for disease features
        disease_agg = th.stack((th.mm(row_normalize(drug_disease.T).float(), F.relu(self.fc_Di_D(drug_feat))),
                               th.mm(row_normalize(protein_disease.T).float(), F.relu(self.fc_Di_P(protein_feat))),
                               disease_feat), dim=1)
        disease_feat, _ = self.att_sum(disease_agg)
        disease_feat = disease_feat + disease_agg.mean(dim=1)  # Residual connection
        
        # Add residual connection for drug features
        drug_agg = th.stack((th.mm(row_normalize(drug_drug).float(), F.relu(self.fc_DDI(drug_feat))),
                             th.mm(row_normalize(drug_chemical).float(), F.relu(self.fc_D_ch(drug_feat))),
                             th.mm(row_normalize(drug_disease).float(), F.relu(self.fc_D_Di(disease_feat))),
                             th.mm(row_normalize(drug_sideeffect).float(), F.relu(self.fc_D_Side(sideeffect_feat))),
                             th.mm(row_normalize(drug_protein).float(), F.relu(self.fc_D_P(protein_feat))),
                             th.mm(row_normalize(drug_substituents).float(), F.relu(self.fc_D_Subs(substituents_feat))),
                             drug_feat), dim=1)
        drug_feat, _ = self.att_sum_1(drug_agg)
        drug_feat = drug_feat + drug_agg.mean(dim=1)  # Residual connection

        # Add residual connection for protein features
        protein_agg = th.stack((th.mm(row_normalize(protein_protein).float(), F.relu(self.fc_PPI(protein_feat))),
                                th.mm(row_normalize(protein_sequence).float(), F.relu(self.fc_P_seq(protein_feat))),
                                th.mm(row_normalize(protein_disease).float(), F.relu(self.fc_P_Di(disease_feat))),
                                th.mm(row_normalize(drug_protein.T).float(), F.relu(self.fc_P_D(drug_feat))),
                                th.mm(row_normalize(protein_go).float(), F.relu(self.fc_P_GO(go_feat))),
                                protein_feat), dim=1)
        protein_feat, _ = self.att_sum_2(protein_agg)
        protein_feat = protein_feat + protein_agg.mean(dim=1)  # Residual connection

        # Add residual connection for sideeffect features
        sideeffect_agg = th.stack((th.mm(row_normalize(drug_sideeffect.T).float(), F.relu(self.fc_Side_D(drug_feat))),
                                   sideeffect_feat), dim=1)
        sideeffect_feat, _ = self.att_sum_3(sideeffect_agg)
        sideeffect_feat = sideeffect_feat + sideeffect_agg.mean(dim=1)  # Residual connection

        # Add residual connection for substituents features
        substituents_agg = th.stack((th.mm(row_normalize(drug_substituents.T).float(), F.relu(self.fc_Subs_D(drug_feat))),
                                     substituents_feat), dim=1)
        substituents_feat, _ = self.att_sum_4(substituents_agg)
        substituents_feat = substituents_feat + substituents_agg.mean(dim=1)  # Residual connection

        # Add residual connection for go features
        go_agg = th.stack((th.mm(row_normalize(protein_go.T).float(), F.relu(self.fc_GO_P(protein_feat))),
                           go_feat), dim=1)
        go_feat, _ = self.att_sum_5(go_agg)
        go_feat = go_feat + go_agg.mean(dim=1)  # Residual connection

        # Concatenate features and apply propagation
        ...
        # Split embeddings and apply normalization
